...
class NLIVerifier:
    ...
    def _heuristic_nli(self, premise: str, hypothesis: str) -> Dict[str, float]:
        ...
        def _extract_numbers_with_contexts(text: str) -> List[Tuple[Any, str, bool]]:
            ...
            # Match 1-15 digit standard integers (avoid matching inside exponents or floats)
            all_ints_matches = list(re.finditer(r"\b\d{1,15}\b", norm_text))

            unc_words = {"uncertainty", "uncertainties", "error", "margin", "tolerance", "deviation", "unc", "relative"}
            has_global_unc = any(w in norm_text.lower() for w in unc_words)
            ...
            for m in float_matches:
                ...
                end_idx = min(len(norm_text), m.end() + 35)
                local_ctx = norm_text[start_idx:end_idx].lower()
                prec_ctx = norm_text[max(0, m.start() - 35):m.start()].lower()
                has_prec_unc = any(w in prec_ctx for w in unc_words)
                raw_results.append((val, local_ctx, has_prec_unc))
            ...
            for m in all_ints_matches:
                ...
                # Skip unit powers/exponents (e.g. m^3, s^-2, 10^5)
                is_unit_power = (m.start() > 0 and norm_text[m.start()-1] in ('^', 'e', 'E')) or \
                                (m.start() > 1 and norm_text[m.start()-2:m.start()] in ('^-', '^+', 'e-', 'e+'))
                if not is_part_of_float and not is_unit_power:
                    try:
                        val = float(val_str)
                    except ValueError:
                        val = val_str
            ...
            results = []
            for val, ctx, has_prec_unc in raw_results:
                is_unc = has_prec_unc or (len(raw_results) == 1 and has_global_unc)
                results.append((val, ctx, is_unc))
            return results
        ...
        if p_entries and h_entries:
            ...
            shared_units = {"m", "kg", "s", "g", "c", "h", "e", "k", "n", "j", "w", "v", "hz", "pa", "t"}

            for val_h, ctx_h, unc_h in h_entries:
                if unc_h:
                    continue
                # If this hypothesis number has a direct match in premise, it is verified and not a clash
                if any(_vals_match(vp, val_h) for vp, _, _ in p_entries):
                    continue

                # Look for an unmatched premise number that occupies the same semantic/contextual slot
                for val_p, ctx_p, unc_p in p_entries:
                    if unc_p:
                        continue
                    # If premise value is already matched by another hypothesis number, skip
                    if any(_vals_match(val_p, vh) for vh, _, _ in h_entries):
                        continue

                    # Compare local contexts
                    tokens_p = {w for w in re.findall(r"\b[a-z0-9_]{1,}\b", ctx_p) if w not in stopwords}
                    tokens_h = {w for w in re.findall(r"\b[a-z0-9_]{1,}\b", ctx_h) if w not in stopwords}

                    has_shared_unit = bool((tokens_p & tokens_h) & shared_units)
                    has_shared_content = bool((tokens_p & tokens_h) - shared_units)

                    p_exp = re.findall(r"e[-+]?\d+", str(val_p))
                    h_exp = re.findall(r"e[-+]?\d+", str(val_h))
                    has_matching_exp = bool(p_exp and h_exp and p_exp == h_exp)

                    if has_shared_content or (has_shared_unit and (has_matching_exp or len(tokens_p & tokens_h) >= 2)):
                        num_clash = True
                        break
                if num_clash:
                    break
