...
class OmniGuardProductionPipeline:
    ...
    def query(self,
              ...
              llm_generator_fn: Optional[Callable[[str, str], str]] = None) -> PipelineExecutionResult:
        """Executes the full end-to-end grounded query pipeline."""
        tracer = PipelineTracer()
        active_tenant = tenant_id or self.tenant_id
        active_llm_generator = llm_generator_fn or self.llm_generator_fn
        quarantined_chunks: List[ProductionChunk] = []
        ...
        if active_llm_generator is not None:
            raw_response = active_llm_generator(prompt_payload["system_prompt"], prompt_payload["user_prompt"])
        else:
        ...
        if enable_cov and verified_chunks:
            ...
            cov_res = self.cov_engine.run_verification(
                ...
                llm_generate_fn=active_llm_generator
            )
